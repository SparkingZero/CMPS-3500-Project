...
def train_neural_network():
    ...
    # Get raw labels before encoding
    target_labels = cleaned_data['Status'].values
    class_labels = np.unique(target_labels)

# Compute class weights (label = AA, AO, etc.)
    class_weights_array = compute_class_weight( class_weight='balanced', classes=class_labels, y=target_labels)

# Map class labels to encoded indices used by Keras
    label_to_index = {label: i for i, label in enumerate(target_encoder.categories_[0])}
    class_weights_dict = { 
        label_to_index[label]: weight for label, weight in zip(class_labels, class_weights_array)
        }
    ...
    # Train
    history = model.fit(X, Y, epochs=30, batch_size=32, validation_split=0.2, class_weight=class_weights_dict, callbacks=[early_stop])
...
def generate_predictions():
    ...
    timestamp(f"Predictions generated (predictionClassProject1.csv)...")
